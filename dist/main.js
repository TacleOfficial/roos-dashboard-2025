(function () {
  ...
  waitForFirebaseReady(async () => {
    ...
    auth.onAuthStateChanged(async (user) => {
      ...
      initTradeshowAdminUI(window.portalCtx);   // ← tradeshow admin viewer
...
async function loadAndShowSampleModal({ db, uid, sampleId }) {
 ...
 // ========== TRADESHOW LEADS ADMIN VIEWER (no innerHTML) ==========
  function initTradeshowAdminUI(ctx) {
    ...
    function subscribeForEvent(eventId) {
      if (unsubscribe) {
        unsubscribe();
        unsubscribe = null;
      }
    ...
    // initial event from URL or input
    try {
    ...
    subscribeForEvent((eventInput.value || '').trim());
